...
(() => {
  ...
  const UI = {
    ...
    // ---------- Auth ----------
    bindAuth() {
      if (USE_SUPABASE && SupaSync.supa) {
        $('#btn-google').addEventListener('click', async () => {
          try { await SupaSync.supa.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: location.href } }); } catch (e) { console.warn(e); }
        });
        $('#email-form').addEventListener('submit', async (e) => {
          e.preventDefault();
          const mail = $('#email-input').value.trim(); if (!mail) return;
          try {
            // magic link (free, sem senha) — ou fallback para mock se não configurado
            const { error } = await SupaSync.supa.auth.signInWithOtp({ email: mail, options: { emailRedirectTo: location.href } });
            if (error) throw error;
            this.toast('Link enviado para ' + mail + ' — verifique seu e-mail', { kind: 'success' });
          } catch (err) {
            Store.setUser({ name: mail.split('@')[0], mail, provider: 'email' });
            this.renderAuthOrApp();
          }
        });
        $('#btn-logout').addEventListener('click', async () => { try { await SupaSync.supa.auth.signOut(); } catch {} Store.setUser(null); this.renderAuthOrApp(); });
        // se já há sessão Supabase, sincroniza Store.user
        SupaSync.supa.auth.getSession().then(({ data: { session } }) => { if (session && session.user) { Store.setUser({ name: session.user.email.split('@')[0], mail: session.user.email, provider: 'supabase', id: session.user.id }); this.renderAuthOrApp(); } });
        return;
      }
      $('#btn-google').addEventListener('click', () => {
        if (USE_SUPABASE) {
          import('https://esm.sh/@supabase/supabase-js@2').then(m => {
            ...
            c.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: location.href } });
          });
          return;
        }
